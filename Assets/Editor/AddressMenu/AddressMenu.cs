...
{
    public static string UpdateGroupName = "UpdateGroup_";
    public static string UpdateGroupNameF = "UpdateGroup_{0}";
    ...
    public static void SetSelectDirectorToGroup()
    {
        ...
        if(folder != null)
            LoopSetAllDirectorToAddress(folder,true);
    }
    ...
    public static void ReBuildAddress()
    {
        ...
        //2.对所有资源进行重新分组打标签
        LoopSetAllDirectorToAddress(GameSettings.GetABRootPath(),true);
        AssetDatabase.Refresh();
    ...
    public static void CheckForUpdateContent()
    {
        ...
        foreach (var temp in entrys){
            sbuider.AppendLine(temp.address);
        }
        Debug.Log(sbuider.ToString());
        //将被修改过的资源单独分组
        var groupName = string.Format(UpdateGroupNameF, DateTime.Now.ToString("yyyyMMddHHmmss"));
        ...
        if (group == null){
            group = CreatAssetGroup<System.Data.SchemaType>(m_Settings, groupName);
        }
        foreach (var temp in entrys) {
            var groupNameTemp = temp.parentGroup.name;
            if (!groupNameTemp.Contains(UpdateGroupName)) {
                m_Settings.MoveEntry(temp, group);
            }
        }
        //else {
        //    m_Settings.MoveEntries(entrys, group);
        //}
    }
    //迭代打包
    ...
    public static void BuildUpdate()
    {
        ...
        OnKeyUpdateAllLuaToTxt();
        //2.对Lua 文件进行重新分组打标签 
        LoopSetAllDirectorToAddress(GameSettings.GetLuaTxtPath(),false);
        //3.对比更新列表
    ...
    private static void LoopSetAllDirectorToAddress(string pFileDirectorRoot,bool pIsNewBuild)
    {
        if (Directory.Exists(pFileDirectorRoot))
        {
            SetDirectorABNameNull(pFileDirectorRoot, pIsNewBuild);
            ...
            if (direcs.Length > 0)
            {
                for (var i = 0; i < direcs.Length; i++)
                {
                    ...
                    {
                        LoopSetAllDirectorToAddress(direcs[i].FullName, pIsNewBuild);
                    }
                }
            }
        }
    }
    private static void SetDirectorABNameNull(string pFileDirectorRoot, bool pIsNewBuild)
    {
        if (Directory.Exists(pFileDirectorRoot))
        {
            var dirctory = new DirectoryInfo(pFileDirectorRoot);
            ...
            for (var i = 0; i < files.Length; i++)
            {
                ...
                AutoGroup(groupName, assetPath, groupName, pIsNewBuild);
                isAdd = true;
            }
            if (isAdd)
                AssetDatabase.Refresh();
    ...
    public static void AutoGroup(string groupName, string assetPath,string pLable,bool pIsNewBuild)
    {
        ...
        if (pIsNewBuild){
            ...
            var entryN = settings.FindAssetEntry(guid);
            if (entryN.parentGroup == null || !entryN.parentGroup.name.Contains(UpdateGroupName)) {
                var entry = settings.CreateOrMoveEntry(guid, group);
                entry.address = assetPath;
                entry.SetLabel(pLable, true, true);
            }
