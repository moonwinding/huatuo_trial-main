...
public class PlayableAssetGO
{
	...
	public GameObject GetGo(PlayableGraph graph, GameObject go,out GameObject outParent) {
		...
		if (go_ == null) {
			if (!string.IsNullOrEmpty(relaPath)){
				var findModel = go.transform.Find(relaPath);
				go_ = findModel == null ? go : findModel.gameObject;
			}
			else {
				go_ = go;
			}
