            ...
            if (isCanSend)
            {
                ...
                if (tool == null || tool.go == null)
                {
                    ...
                    if (timeLineInfer != null) {
                        mRec.transform.forward = (timeLineInfer.target.position - target_.transform.position).normalized;
                    }
                    mRec.transform.position = new Vector3(target_.transform.position.x, Height, target_.transform.position.z);
