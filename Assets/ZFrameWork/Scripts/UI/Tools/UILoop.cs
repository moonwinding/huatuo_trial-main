...
public class UILoop : MonoBehaviour
{
    ...
    public void SetData(List<object> datas,System.Action<GameObject, object> pSetFunc)
    {
        m_Datas = datas as IList;
        updateItemFunc = pSetFunc;
    ...
    private void updateItem(int index, GameObject item)
    {
        ...
        if (item.activeSelf)
        {
            ...
            updateItemFunc.Invoke(item, data);
        }
    }
#if UNITY_EDITOR
    [Button("测试组件")]
